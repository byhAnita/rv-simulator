...
export default function KakaoOverlay({ memberId, members, kktMessages, kktUnlocked, allTargetMembers, onClose }) {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,.75)", backdropFilter: "blur(4px)" }}>
      <div style={{ width: "100%", maxWidth: 360, height: "80vh", maxHeight: 600, background: "#b2c7d9", borderRadius: 20, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,.5)" }}>
        <div style={{ background: "#3c1e1e", padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          ...
          <span style={{ color: "#fff", fontSize: 14, fontWeight: 600 }}>카카오톡</span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 10px", display: "flex", flexDirection: "column", gap: 8 }}>
          {!unlocked ? (
            <div style={{ textAlign: "center", color: "#888", padding: "30px 0", fontSize: 12 }}>
              🔒 尚未添加{m?.name}为KKT好友<br />
              <span style={{ fontSize: 10 }}>好感达{KKT_THRESHOLD}后解锁</span>
            </div>
          ) : msgs.length === 0 ? (
            <div style={{ textAlign: "center", color: "#888", padding: "30px 0", fontSize: 12 }}>暂无消息</div>
          ) : (
            msgs.map((msg, i) => (
              <div key={i} style={{ display: "flex", justifyContent: msg.from === "player" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 5 }}>
                {msg.from !== "player" && (
                ...
                <div style={{ maxWidth: "70%", background: msg.from === "player" ? "#fee500" : "#fff", color: "#1a1a1a", padding: "7px 10px", borderRadius: msg.from === "player" ? "12px 3px 12px 12px" : "3px 12px 12px 12px", fontSize: 12, lineHeight: 1.5, wordBreak: "break-word" }}>{msg.content}</div>
              </div>
            ))
          )}
        </div>
